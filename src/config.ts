...
const notifications = {
  ...
  discord: {
    ...
    notifyGroupSeries: {
      'x16': envOrArray(process.env.DISCORD_NOTIFY_GROUP_X16),
...
const store = {
  ...
  maxPrice: {
    series: {
      'x16': envOrNumber(process.env.MAX_PRICE_SERIES_X16),
  ...
  //stores: envOrArray(process.env.STORES, ['amazon', 'bestbuy','walmart']).map(entry => {
  stores: envOrArray(process.env.STORES, ['commanderx16']).map(entry => {
